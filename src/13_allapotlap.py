...
Futtatás:  python src/13_allapotlap.py
Előtte:    matlab -batch "cd('src'); smoke_test"   (ez írja a t00-t)
"""
...
def main() -> None:
    ...
    if hianyzo:
        sys.exit("Hiányzó bemenet:\n  " + "\n  ".join(map(str, hianyzo)) +
                 "\n\nA t00-hoz futtasd: matlab -batch \"cd('src'); smoke_test\"")
    ...
    w("       matlab -batch \"cd('src'); smoke_test\"")
    w("       python src/13_allapotlap.py                                -->")
    w("")
    ...
    w("")
    w("**Fő modell:** `src/model/jv_dsge_v09_access.mod` (Jakab–Világi mag). "
