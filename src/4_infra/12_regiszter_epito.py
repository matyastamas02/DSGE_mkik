...
Futtatás:  python src/4_infra/12_regiszter_epito.py --megerosit
"""
...
def main() -> None:
    if "--megerosit" not in sys.argv:
        sys.exit(
            ...
            "Ha tényleg újra akarod építeni: python src/4_infra/12_regiszter_epito.py --megerosit"
        )

    dump = REG / "_params_dump.csv"
    if not dump.exists():
        sys.exit(f"Hiányzik: {dump}\n"
                 "Előbb: matlab -batch \"cd('src/modell/1_fo_vonal_jv/futtato'); "
