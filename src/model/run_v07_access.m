% run_v07_access.m — haromszektoros modell hitelhozzaferesi margoval
%
% Kimenet:
%   output/tables/t29_v07_access_szcenariok.csv
%   output/tables/t29_v07_access_hosszutav.csv
%   output/figures/f24_v07_access_szcenariok.png
%
% Futtatas:
%   matlab -batch "cd('<repo>/src/model'); run_v07_access"
...
valt = {'y','yd','y_E','y_D','y_L','x_E','x_D','x_L', ...
    'i_E','i_D','i_L','efp_E','efp_D','efp_L','acc_E','acc_D', ...
    'xx','c','infl','r','rer','bstar'};
osszes = table(); ht = table();

for s = 1:3
    dynare('kkv_dsge_v07_access', sprintf('-DSCENARIO=%d', s), ...
        '-DTSCEN=3', 'console');
...
% [repo-illesztes, 2026-08-12] Az eredeti 'repo = pwd;' a szerzo MATLAB
% Drive-elrendezeset feltetelezte (ott a pwd a repo gyokere volt). Ebben a
% repoban a futtato a src/model-bol indul, ezert ket szintet feljebb kell
% lepni -- ugyanugy, ahogy a run_jv_v0N.m scriptek csinaljak. A MODELL
% (kkv_dsge_v07_access.mod) ERINTETLEN.
...
writetable(osszes, fullfile(tablazatok, 't29_v07_access_szcenariok.csv'));
writetable(ht, fullfile(tablazatok, 't29_v07_access_hosszutav.csv'));
...
annotation(fig, 'textbox', [0.01 0.005 0.98 0.045], ...
    'String', ['v07_access: v06_3type + redukalt KKV hitelhozzaferesi margin. ' ...
...
sgtitle(fig, 'kkv\_dsge\_v07\_access — export-KKV / hazai KKV / nagyvallalat', ...
    'FontSize', 12, 'Color', tinta);
exportgraphics(fig, fullfile(abrak, 'f24_v07_access_szcenariok.png'), ...
    'Resolution', 180);

fprintf('Kiirva: t29_v07_access_* + f24_v07_access_szcenariok.png\n');
