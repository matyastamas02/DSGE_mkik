/*
 * kkv_dsge_v05.mod — Calvo-bérek a v0.4-re építve
 * =====================================================================
 * A v0.4 (kamatunió-rezsimváltás + 75% nem-Ricardiánus háztartás)
 * kiegészítése ragadós bérekkel, EHL (Erceg–Henderson–Levin) típusú
 * bér-Phillips-görbével, CPI-indexálással:
 *
 *   piw − chiw·infl(−1) = β·[piw(+1) − chiw·infl] + kap_w·(mrs − w)
 *   w = w(−1) + piw − infl          (reálbér-azonosság)
 *   mrs = sigma_n·nn − lam          (határáldozat)
 *
 * kap_w = (1−θw)(1−βθw) / [θw·(1+σ_n·η_w)], ahol η_w = 4.33 a bér-markup
 * elaszticitás (EAGLE 5. tábla). Kalibráció a WP 2017/7 + Kézdi–Kónya
 * (WDN) alapján: θw = 0.75 (a cégek ~évente igazítanak alapbért),
 * indexálás chiw = 0.75.
 *
 * Kapcsolók:  -DTHETAW=60|75|85 (Calvo-bér, %-ban; alap 75 — érzékenység)
 *             -DOMNR=0|75, -DSCENARIO=1|2|3 (mint v0.4)
 * Futtatás:   run_v05.m (v0.4 rugalmas-bér vs. v0.5 érzékenységi sáv)
 */
...
var
    c c_R c_N lam w nn r infl inflH y ii xx imp rer dep bstar piw mrs
...
theta_w = @{THETAW}/100; // Calvo-bér (EAGLE: 0.75; Kézdi-Kónya: ~évente)
chiw  = 0.75;            // bérindexálás (EAGLE 2. tábla)
eta_w = 4.33;            // bér-markup elaszticitás (EAGLE 5. tábla)
...
// Calvo-bérek (EHL): bér-Phillips-görbe + reálbér-azonosság
mrs = sigma_n*nn - lam;
