...
% Futtatás:  matlab -batch "cd('<repo>/src/modell/3_archiv_korai_jv/futtato'); run_jv_v06"
%
...
% EZ AZ EGYETLEN FUTTATO, AMI KET MAPPABOL HASZNAL MODELLT: a jv_dsge_v06
% mar a FO VONALHOZ tartozik (1_fo_vonal_jv), a jv_dsge_v05 viszont itt
% maradt az archivumban -- a script epp a ketto OSSZEVETESE. Ezert nem
% eleg egy cd: mindket .mod mappajat kulon tartjuk, es a dynare-hivas elott
% odalepunk. A repo gyokeret a script sajat helyebol szamoljuk.
itt   = fileparts(mfilename('fullpath'));
repo  = itt;
while ~isfile(fullfile(repo, 'CLAUDE.md')), repo = fileparts(repo); end
ut_v05 = fullfile(repo, 'src', 'modell', '3_archiv_korai_jv');
ut_v06 = fullfile(repo, 'src', 'modell', '1_fo_vonal_jv');
assert(isfile(fullfile(ut_v05, 'jv_dsge_v05.mod')), 'Hianyzik a jv_dsge_v05.mod');
assert(isfile(fullfile(ut_v06, 'jv_dsge_v06.mod')), 'Hianyzik a jv_dsge_v06.mod');
...
% [a repo-t a fejlec mar beallitotta -- a pwd itt mar a v05 mappaja, tehat
%  a regi fileparts(fileparts(pwd)) rossz szintre mutatna]
