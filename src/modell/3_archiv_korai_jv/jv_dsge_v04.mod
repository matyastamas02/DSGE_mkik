/*
 * jv_dsge_v04.mod — Közgazdaságilag tartalmas KKV/nagyvállalat szegmentálás
 * =====================================================================
 * A jv_dsge_v02 (JV-mag + kétszektoros BGG) BK-BIZTOS szerkezetére épül,
 * és hozzáteszi a csapatdöntés szerinti VERTIKÁLIS (beszállítói) linket.
 * (Egy korábbi, szektor-specifikus tőkés + CPI-szétválasztásos változat
 * megbontotta a Blanchard–Kahn feltételt; ez a robusztus verzió.)
 *
 *   KKV        ≙ HAZAI szektor: k_S tőke, MAGAS EFP (chi_S), RUGALMAS
 *                (alacsony psi_i_S), hazai piac + beszállít az exportőrnek.
 *   Nagyvállalat ≙ EXPORT szektor: k_L, ALACSONY EFP (chi_L), MEREV
 *                (magas psi_i_L), exportkapu.
 *
 * VERTIKÁLIS LINK (a fő újítás), két csatornán:
 *  (a) KÖLTSÉG: az export határköltsége tartalmazza a KKV-input árát,
 *      amit a KKV határköltsége (mc_d) ad, s_kkv költséghányaddal:
 *        mcx = (1-s_kkv)*(export value-added ktsg) + s_kkv*mc_d - px
 *      -> olcsóbb KKV-hitel -> több KKV-tőke -> alacsonyabb rk és mc_d
 *         -> olcsóbb input az exportőrnek.
 *  (b) MENNYISÉG: a KKV-jószág kereslete tartalmazza az exportőr
 *      input-keresletét (h_dx), ami az exporttal (xx) skálázódik.
 * Így az euró-hitelsokk POZITÍV ÖSSZEGŰ: a KKV egészsége az EXPORTŐRT is
 * segíti. Az eredmény egyensúly, nem "a gyengébb meghal" mechanika.
 *
 * Adat (Opten t02): 250+ fős 70% exportőr, 10-49 fős 6% -> nagyvállalat=
 * exportkapu, KKV=hazai leképezés indokolt.
 *
 * KALIBRÁCIÓ (2-es döntés): s_kkv, mu_vert, psi_i_S/L IRODALMI/becsült
 * induló érték, ÉRZÉKENYSÉGKÉNT; a pontos KKV-input arány a KSH ágazati
 * kapcsolatok mérlegéből (IO-tábla) pótolandó.
 *
 * Futtatás: matlab -batch "cd('<repo>/src/modell/3_archiv_korai_jv'); dynare jv_dsge_v04 console"
 */
...
parameters
    ...
    lam_p lam_x lam_w
    hx mu_x gam_i phi_pi nu_b om_no
...
// --- ÚJ: szegmentálás-paraméterek (irodalmi/becsült induló, érzékenység) ---
psi_i_S = 8.0;        // KKV RUGALMASABB (alacsonyabb beruh. kiigazítás)
psi_i_L = 13.0;       // nagyvállalat merev
s_kkv   = 0.20;       // KKV-input költséghányad az exportban (IO-táblából pótolandó)
mu_vert = 0.50;       // az exportőr KKV-input keresletének árrugalmassága
sc = 0.54; si = 0.23; sg = 0.10; sx = 0.60; sm = 0.47;
sh_ld = 0.70; sh_kd = 0.65; sh_imd = 0.30;
// a hazai (KKV) jószág felhasználási arányai (összeg = 1)
...
// --- 2. Kétszektoros BGG (v02 BK-biztos szerkezet; psi eltér típusonként) ---
...
i_S = 1/(1+beta)*i_S(-1) + beta/(1+beta)*i_S(+1)
      + 1/((1+beta)*psi_i_S)*q_S + e_i_ar;    // KKV rugalmasabb
i_L = 1/(1+beta)*i_L(-1) + beta/(1+beta)*i_L(+1)
      + 1/((1+beta)*psi_i_L)*q_L + e_i_ar;    // nagyvállalat merev
k_S = (1-delta)*k_S(-1) + delta*i_S;
k_L = (1-delta)*k_L(-1) + delta*i_L;
k  = om_S*k_S + (1-om_S)*k_L;
ii = om_S*i_S + (1-om_S)*i_L;

// --- 3. Termelés (v02 szerkezet, közös rk) + VERTIKÁLIS export-input ---
wz_d = a_d*w + (1-a_d)*rer;
wz_x = a_x*w + (1-a_x)*rer;
mc_d = zeta_d*rk + (1-zeta_d)*wz_d - a;
// export határköltség: value-added ktsg + KKV-input (ára = mc_d), s_kkv súllyal
...
// KKV-input kereslet az exportőrtől: az exporttal skálázódik, a relatív
// költségtől (mc_d vs. export ktsg) függ
...
// --- 5. Kereslet, külkereskedelem ---
// KKV (hazai) jószág: fogyasztás + KKV-beruházás + kormányzat + export-input
