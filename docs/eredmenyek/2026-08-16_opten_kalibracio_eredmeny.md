...
> **KORREKCIÓ — 2026-08-24 (valódi Blanchard–Kahn-audit).** A dokumentum
> eredeti `konvergalt` jelzője kizárólag a perfect-foresight solver numerikus
> sikerét mérte, nem a BK-feltételt. A `t47` rácson a solver továbbra is
> 36/36 esetet old meg, de a **terminális, `uni=1` lokális rezsimben csak
> 9/36 konfiguráció BK-valid**. Az `OPTEN=0` ág 9/9 esetben pontosan 13
> instabil gyököt ad a 13 előretekintő változóhoz. Az `OPTEN=1/2/3` ágak
> viszont együtt 27/27 esetben hibásak: 15 instabil gyök jut 13
> előretekintő változóra. Ezért az ezekhez tartozó, `ACCSCALE=100` melletti
> GDP- és szegmensszintek, valamint a rájuk épülő sávértelmezések **nem
> értelmezhetők modell-eredményként**. Az alábbi szöveg történeti állapotot
> őriz; ahol „36/36 BK-stabil” szerepel, ott helyesen „36/36 PF-solver
> sikeres, 9/36 terminális BK-valid” értendő.
...
| `-DOPTEN=3` | **csak** a magas `rho_acc` érzékenységi érték — dekompozíciós ág |
...
## 4. A legnagyobb mechanikai érzékenység — `rho_acc`
...
Ez a 0,9673-as érték **nem a modell szegmensszintű dinamikus
perzisztenciájának empirikus horgonya**. A négyéves panelben a cégek 92,4%-a
egyszer sem vált hitelstátuszt, ezért a mutató nagyrészt tartós cégek közötti
heterogenitást kever össze időbeli alkalmazkodással. A `rho_acc` így
horgonyzatlan marad; a 0,9673 csak magas-perzisztenciájú érzékenységi ág.

**Miért számít ennyire mechanikailag:** a modellben a hosszú távú hozzáférési hatás
...
A `0 → 3` lépés tisztán a magas-`rho_acc` érzékenység hatása: **a küszöb
...
`1/(1−ρ)` a ρ → 1 határon robban. Egy ilyen, jelenleg horgonyzatlan
paraméterre egyetlen szám nem elég:
...
| GDP @ `ACCSCALE=100` | 0,66% | 0,77% | — | — | — | — |

A küszöb minden felsorolt ponton BK-valid és monoton csökken. Az
`ACCSCALE=100` GDP-pont azonban csak `rho_acc=0,85` és `0,90` mellett
BK-valid; `rho_acc>=0,93` esetén 15 instabil gyök jut 13 előretekintő
változóra, ezért a régi pontértékeket nem közöljük.

---

## 5. ⚠ Amit vissza kellett vonni: a minden ágra kiterjesztett GDP-sáv
...
Az új BK-audit alapján az összehasonlítás helyes formája (`t47`):

| Ág | aggregált GDP-sáv (SCENARIO 1–3 × TSCEN 1–3) |
|---|---|
| `-DOPTEN=0` (átvett) | +0,52% … +1,18% |
| `-DOPTEN=3` (magas `rho_acc`) | **nem közölhető: 0/9 terminális BK-valid** |
| `-DOPTEN=1` (teljes Opten) | **nem közölhető: 0/9 terminális BK-valid** |
| `-DOPTEN=2` (küszöb-25% def.) | **nem közölhető: 0/9 terminális BK-valid** |

Az egyetlen teljes, pontértékekre is értelmezhető ág az `OPTEN=0`: ott a
9/9 kombináció BK-valid, és az aggregált GDP-hatás **+0,52% … +1,18%**.
Az `OPTEN=1/2/3` alatt korábban közölt `ACCSCALE=100` sávokat visszavonjuk;
azok perfect-foresight solver-kimenetek voltak egy lokálisan indeterminált
terminális rezsimben, nem modell-eredmények.
...
de ez önmagában **nem** a hiba oka — az `x_D`-t a saját
exportkereslet-egyenlete továbbra is meghatározza. A teljes rács 36/36
esetben solver-sikeres, de terminálisan csak az `OPTEN=0` ág 9/9 pontja
BK-valid; az `OPTEN=1/2/3` ágak egyaránt 0/9-ek.
...
Amit mértünk: mennyire perzisztens **egy cég** hitelstátusza. Amit a modell
használ: mennyire perzisztens **a szegmens hozzáférési sokkja**. A kettő
nem azonos. Ráadásul a cégek 92,4%-a a négyéves ablakban egyszer sem vált
státuszt, így a 0,9673 nagyrészt fix céghatást is tartalmaz. Sem
pontbecslésként, sem alsó korlátként nem használható a modellbeli
`rho_acc`-ra. Ez indokolja a `t49` érzékenységi scant és azt, hogy a
paramétert horgonyzatlanként tartjuk nyilván.
...
| Perfect-foresight solver: 4 ág × 3 SCENARIO × 3 TSCEN | **36/36** |
| Terminális BK: `OPTEN=0` / `OPTEN=1,2,3` | **9/9 / 0/27** |
| Küszöb-sorrend `D < súlyozott KKV < E` minden ágon | tart |
| Küszöb monotonitása a `rho_acc`-ban | monoton csökkenő |
| GDP-hatás a `rho_acc`-ban | csak a BK-valid `rho=0,85; 0,90` pontokon közölhető |
| Súlyok 1-re összegződése (`om_j`, `shl_j`) | 1e−6 alatt |
| Füstteszt | **148 rendben, 0 hiba** |
...
1. **A `lev_E > lev_D` a projekt egyik kevés, saját adaton nyugvó,
   ...
   ugyanaz.
2. **A magas-`rho_acc` érzékenységi ágon a KKV-küszöb feleződik**, de a küszöb**forma** marad: az `ACCSCALE`
   ...
   **mi viszi** a küszöb szintjét. A `rho_acc` továbbra is horgonyzatlan.
3. **Az aggregált sávot leszűkítettük a BK-valid `OPTEN=0` ágra**
   (+0,52% … +1,18%); a többi ág pontértékeit visszavontuk.
